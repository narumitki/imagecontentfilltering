...
def draw_bbox(image, bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False):   
    ...
    print(NUM_CLASS[class_ind]," Score:",score_str)
    return image
...
def draw_bboxV2(image, bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False):   
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if show_label:
            ...
            bbx_class = [NUM_CLASS[class_ind],score_str,[x1,y1,x2,y2]]
            bbox_class_list.append(bbx_class)
    #print("return --> [NUM_CLASS[class_ind],score_str,[x1,y1,x2,y2]]")
    return image,bbox_class_list
...
def detect_imageV2(Yolo, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    original_image      = cv2.imread(image_path)
    original_image      = cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB)
    original_image      = cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB)
    ...
    image,bbox_class_list = draw_bboxV2(original_image, bboxes, CLASSES=CLASSES, rectangle_colors=rectangle_colors)
    # CreateXMLfile("XML_Detections", str(int(time.time())), original_image, bboxes, read_class_names(CLASSES))
...
def detect_image_API(Yolo, image, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
